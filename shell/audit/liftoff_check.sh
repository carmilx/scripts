...
# Logging config
LOG_NAME="kandji_liftoff_audit.log"
...
if pgrep -q "Liftoff"; then
 	logging "Liftoff is running, remediation will run the dock script"
 	exit 1
elif [ "$(( $(date +"%s") - $(stat -f "%B" /Users/"$loggedInUser") ))" -gt "86400" ]; then
    logging "Home folder is older than 24 hours, assume dock already tidied."
    exit 0 # do not tidy dock
else
 	logging "Liftoff is not running, no remediation needed"
 	exit 0
fi